...
func (s *Service) GetSimilarity(pubkey string) ([]string, error) {
	// Get similar users sorted by similarity score
	users, err := s.neo4j.ExecuteRead(func(tx neo4j.ManagedTransaction) (any, error) {
		ctx := context.Background()

		// Match users connected by SIMILAR relationship to given pubkey
		// Order by similarity score descending
		result, err := tx.Run(ctx, `
			MATCH (u:User {pubkey: $Pubkey})-[s:SIMILAR]->(similar:User)
			RETURN similar.pubkey, s.score
		...
		for result.Next(ctx) {
			...
			similarPubkey := record.Values[0].(string)
			users = append(users, similarPubkey)
		}
	...
	if err != nil {
		logger.Error("Failed to get similar users", "err", err)
		return nil, err
	}
